...
class Retrieve_Data:
    def __init__(self):
        ...
        self.df_projects['AFOLU Activities'].fillna(self.df_projects['Project Type'], inplace=True)

        self.project_merge = self.df_projects.copy()
        self.project_merge['AFOLU Activities'].fillna(self.project_merge['Project Type'], inplace=True)
        self.project_merge = self.project_merge[['Project ID','AFOLU Activities']]
        self.project_merge.columns = ['Project ID','Method']
    ...
    def retirement_ratios(self):
        ...
        xx['Rolling_Retirement'] = xx.groupby('Method')['Quantity of Units'].transform(lambda i: i.expanding().sum())
        xx = xx.drop(columns='Quantity of Units')    
        
        z_dates = list(zz.YY_MM.unique())
        x_dates = list(xx.YY_MM.unique())
        all_dates = z_dates + x_dates
        all_dates = list(dict.fromkeys(all_dates))
        all_dates.sort()
        all_dates = [i for i in all_dates if str(i) != 'n_nan']
        
        raw = pd.DataFrame()
        raw['YY_MM'] = all_dates
        raw = pd.merge(raw, xx, on='YY_MM', how="left")
        raw = pd.merge(raw, zz, on=['YY_MM','Method'], how="left")
        raw = raw.sort_values(by=['Method','YY_MM']).reset_index(drop=True)
        raw = raw.fillna(0)
        raw['Rolling_Issuance'] = raw.groupby('Method')['Quantity of Units Issued'].transform(lambda i: i.expanding().sum())
        raw = raw.drop(columns='Quantity of Units Issued')
        raw['Retirement Ratio'] = raw.Rolling_Retirement / raw.Rolling_Issuance
        return raw
...
rat = app.retirement_ratios()
ngeo_issuance = app.ngeo_issuance
df_iss = app.df_issuance
df_ret = app.df_retirement
